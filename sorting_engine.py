...
class MusicSorter:
    ...
    def scan_directory(self):
        # Scan the specified music directory for files
        return [os.path.join(dp, f) for dp, dn, filenames in os.walk(self.music_directory) for f in filenames if not f.startswith('.') and not f.endswith('.pyc') and not f.endswith('.py')]
